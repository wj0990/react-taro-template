...
> = (props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    submit: () => {
      // console.llog('-----uploadQueueuploadQueueuploadQueue---->',uploadQueue)
  ...
  const onDeleteItem = (file: FileItem, index: number, event?:MouseEvent) => {
    event?.stopPropagation();
    event?.preventDefault();
    clearUploadQueue(index)
  ...
  const handleItemClick = (file: FileItem, event?: ITouchEvent) => {
    event?.stopPropagation();
    event?.preventDefault();
        ...
        <div className="nut-uploader__slot">
          <Block>
            ...
            {maxCount > fileList.length && (
              <Button className="nut-uploader__input" onClick={(event)=>_chooseImage(event)} />
            )}
          </Block>
        </div>
      )}
                      ...
                      <div className="nut-uploader__preview__progress">
                        {item.failIcon !== ' ' &&
                          ...
                          (item.status === 'error'
                            ...
                            : item.loadingIcon || <Failure color="#fff" name="loading" />)}
                    ...
                    <Block>
                      {previewUrl ? (
                        ...
                        <div className="nut-uploader__preview-img__file">
                          ...
                          >
                            <Failure name="link" color="#808080"></Failure>
      ...
      {maxCount > fileList.length && previewType === 'picture' && !children && (
        ...
        >
          <div className="nut-uploader__icon">
            {uploadIcon || <Failure name="photograph" size="20px" color="#808080" />}
            {/* {uploadIcon || <Photograph size="20px" color="#808080" />} */}
            <span className="nut-uploader__icon-tip">{uploadLabel}</span>
          </div>
          <Button data-action="chooseImage" className="nut-uploader__input" onClick={(event)=>{
            _chooseImage(event)}
          } />
        </div>
      )}
